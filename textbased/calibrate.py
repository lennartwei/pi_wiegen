...
def printAll(rawBytes):
    ...
    print(
        f"[INFO] INTERRUPT_BASED | longValue: {longValue} | longWithOffsetValue: {longWithOffsetValue} | weight (grams): {weightValue}")
...
def getRawBytesAndPrintAll():
    ...
    weightValue = hx.rawBytesToWeight(rawBytes)
    print(
        f"[INFO] POLLING_BASED | longValue: {longValue} | longWithOffsetValue: {longWithOffsetValue} | weight (grams): {weightValue}")
...
referenceUnit = -148062.5 / 358 #-320795.0 / 803.2  # -194051.5/485.9 #works 2x MSB #-152606.0/382.5 #works 2x MSB   #13645748.0/485.9
...
while True:
    try:
        if READ_MODE == READ_MODE_POLLING_BASED:
            ...
            rawBytes = hx.getRawBytes()
            array.append(hx.rawBytesToLongWithOffset(rawBytes))
            # print(hx.rawBytesToLongWithOffset(rawBytes))
            if len(array) == 100:
